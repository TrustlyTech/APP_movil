...
class EstadisticasService {
  ...
  /// Obtiene estadísticas de denuncias según el intervalo especificado.
  /// Intervalos válidos: 'mes', '6M', 'YTD', '1Y', 'dia', 'anio'.
  static Future<List<DenunciaEstadistica>> getDenunciasPorPeriodo(String intervalo) async {
    final response = await http.get(
      Uri.parse('$baseUrl/estadisticas/denuncias?intervalo=$intervalo'),
    );
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Error al obtener estadísticas de denuncias');
    }
  }

  /// Obtiene estadísticas por localización (ciudad o país).
  static Future<List<LocalizacionEstadistica>> getEstadisticasLocalizacion(String tipo) async {
    final response = await http.get(
      Uri.parse('$baseUrl/estadisticas/localizacion?tipo=$tipo'),
    );
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Error al obtener estadísticas de localización');
    }
  }

  /// Obtiene el top 5 de requisitoriados más denunciados.
  static Future<List<TopRequisitoriado>> getTopRequisitoriados() async {
    final response = await http.get(
      Uri.parse('$baseUrl/estadisticas/top_requisitoriados'),
    );
